...
const items = [{ id: "a", beginnerVariant: "a" }, { id: "b", beginnerVariant: "b" }, { id: "c" }];
assert(Object.keys(model.beginnerVariants).length === 2, "Zwei Einsteigerpfade werden erwartet");
assert(model.visibleItems(items, null, { mode: "beginner" }).length === 2, "Einsteigermodus zeigt nicht genau beide Baupfade");
assert(model.visibleItems(items, "c", { mode: "beginner" }).length === 2, "Fremde Auswahl erweitert den Einsteigerkatalog");
